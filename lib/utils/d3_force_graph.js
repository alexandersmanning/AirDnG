...
export const createForceGraph = function(wordList, svgDOM) {
	...
	let tip = d3tip
		...
		.offset([-10, 0])
		.html(function(d) {
			let name = d.id.split(' ').map(el => el.charAt(0).toUpperCase() + el.slice(1)).join(' ');//d.id.charAt(0).toUpperCase() + d.id.slice(1)
			return "<div class='tool-tip-box'><div class='node-name'>"+ name +"</div><div class='freq'>Mentions: "+ d.count+"</div><div class='tt-tail'></div></div>"
		})	
